...
USE_KL="algorithm.kl_ctrl.kl_coef=0.001 actor_rollout_ref.actor.kl_loss_coef=0.001"

LOCAL_PATH="/root/autodl-tmp/mn-comp"
LOG_DIR="/root/RAGEN/logs/mn-comp"
MODEL_PATH="/root/autodl-tmp/mn-comp/mix100"
mkdir -p "$LOG_DIR" # 如果目录不存在，则创建它

# 获取当前时间，格式为 YYYY-MM-DD-HHMMSS
TIMESTAMP=$(date +"%m-%d-%H-%M")

WANDB_MODE=offline RAY_DEDUP_LOGS=0 python train.py --config-name _15_NashMix \
 ...
 trainer.experiment_name=MixNash-kl \
 $USE_GRPO $USE_KL 2>&1 | tee "$LOG_DIR/grpo_${TIMESTAMP}.log" &
